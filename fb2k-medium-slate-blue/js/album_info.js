...
const elements = {
  ...
  badgeElement: {
    ...
    w: 0,
    h:
      _getFontLineHeight(TS.label.font) +
...
const SECTIONS = [
  ...
  },
  {
    ...
    getContentHeight() {
      return Math.max(
        elements.badgeElement.h,
        _getFontLineHeight(TS.body.font),
      );
    },
    draw(gr) {
      drawBadgeSection(gr, this);
    },
  },
  {
    name: "artist",
    padding: { left: _scale(10), top: 0, right: _scale(10), bottom: _scale(6) },
    icon: iconMgr.get("ui", "artist"),
    iconGap: _scale(5),
    rect: { x: 0, y: 0, w: 0, h: 0 },
    content: { x: 0, y: 0, w: 0, h: 0 },
    visible: true,
  ...
  },
  {
    ...
    draw(gr) {
      drawIconTextSection(
        gr,
        this,
        albumData.genres || "Unknown Genre",
        TS.body.font,
        LEFT_WRAP_FLAGS,
      );
    },
  },
  {
    name: "dateLang",
    padding: { left: _scale(10), top: 0, right: _scale(10), bottom: _scale(6) },
    icon: iconMgr.get("ui", "date"),
    iconGap: _scale(5),
    icon2: iconMgr.get("ui", "language"),
    colGap: _scale(100),
    rect: { x: 0, y: 0, w: 0, h: 0 },
    content: { x: 0, y: 0, w: 0, h: 0 },
    visible: true,
...
// TitleFormat 定义
const albumKeyTf = fb.TitleFormat(
  "%date%%album%$meta(EDITION)$meta(discsubtitle)",
);
...
function reloadAlbumData(metadb) {
  ...
  if (currentAlbumKey === safeAlbumKey) {
    if (panelW > 0) {
      const latestSource = albumSourceTf
...
function drawBadgeSection(gr, sec) {
  ...
  if (albumData.edition) {
    const tx = cx + ICON_SIZE + sec.iconGap;
    _drawText(gr, TS.bodyLine, albumData.edition, tx, cy, editionW, ch);
  }
  if (elements.sourceIcon.img) {
    gr.SetInterpolationMode(7);
    gr.DrawImage(
      elements.sourceIcon.img,
      elements.sourceIcon.x,
      elements.sourceIcon.y,
      elements.sourceIcon.w,
      elements.sourceIcon.h,
      0,
      0,
      elements.sourceIcon.img.Width,
      elements.sourceIcon.img.Height,
    );
  }
  if (currentAQBadge) {
    gr.SetSmoothingMode(4);
    gr.FillRoundRect(
      elements.badgeElement.x,
      elements.badgeElement.y,
      elements.badgeElement.w,
      elements.badgeElement.h,
...
function drawTabSection(gr, sec) {
  ...
  gr.GdiDrawText(
    dBtn.displayText,
    isDescMode ? TS.tab.font : TS.body.font,
    dColor,
    dBtn.x,
    dBtn.y,
    dBtn.w,
...
function updateLayoutMetrics() {
  ...
  if (albumData.edition) {
    editionW =
      _measureText(albumData.edition, TS.bodyLine, lineW).Width + _scale(1);
  }
  if (currentAQBadge) {
  ...
  if (albumData.edition) {
    ...
    if (elements.sourceIcon.img)
      maxW -= THEME.CFG.SOURCE_ICON_SIZE + sec.itemGap;
  ...
  if (SEC.badge.visible) {
    ...
    elements.sourceIcon.y =
      badgeCy + Math.ceil((badgeCh - elements.sourceIcon.h) / 2);
    if (currentAQBadge) {
      elements.badgeElement.x = elements.sourceIcon.img
        ? iconX + THEME.CFG.SOURCE_ICON_SIZE + sec.itemGap
        : iconX;
      elements.badgeElement.y =
        badgeCy + Math.ceil((badgeCh - elements.badgeElement.h) / 2);
    }
    // 合并 source icon + AQ badge 为一个 hit-test 区域 (避免 tooltip 闪烁)
    const gLeft = elements.sourceIcon.img
      ? elements.sourceIcon.x
      : elements.badgeElement.x;
    const gRight = currentAQBadge
      ? elements.badgeElement.x + elements.badgeElement.w
      : elements.sourceIcon.x + elements.sourceIcon.w;
  ...
  elements.descBtn.y =
    SEC.tab.content.y + Math.ceil((SEC.tab.content.h - elements.descBtn.h) / 2);
...
function createTextBuffer() {
  ...
  currentText = isShowingTracklist
    ...
    : albumData.description ||
      "No description available (requires ALBUMDESCRIPTION tag)";
...
function ensureCarouselImageReady(nextIndex, carouselState, reason) {
  ...
  if (
    !carouselState.fallbackMetadb ||
    !carouselState.rawTypes ||
    carouselState.rawTypes.length <= index
  )
    return false;
...
function on_mouse_move(x, y) {
  ...
  if (target) {
    ...
    if (target === elements.badgeGroup) {
      ...
      if (currentAQBadge && currentAQBadge.desc)
        parts.push(currentAQBadge.desc);
      tooltip(parts.join(" · "));
    } else {
