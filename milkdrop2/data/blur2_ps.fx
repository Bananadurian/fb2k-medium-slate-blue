    ...
    out float4 ret : COLOR0)
{
    ...
    //const float d1 = 0.0 + 2.0 * ((w[2] + w[3]) / w1);
    //const float d2 = 2.0 + 2.0 * ((w[6] + w[7]) / w2);
    //const float w_div = 1.0 / ((w1 + w2) * 2.0);
...
#define edge_darken_c3 _c6.w
    ...
    float2 uv2 = uv.xy + srctexsize.zw * float2(1.0, 0.0); // + moves blur UP, LEFT in 2-pixel increments since texture is half the size of "blur1_ps"

    float3 blur =
        (tex2D(sampler_main, uv2 + float2(0.0, d1 * srctexsize.w)).xyz
         + tex2D(sampler_main, uv2 + float2(0.0, -d1 * srctexsize.w)).xyz) * w1 +
        (tex2D(sampler_main, uv2 + float2(0.0, d2 * srctexsize.w)).xyz
         + tex2D(sampler_main, uv2 + float2(0.0, -d2 * srctexsize.w)).xyz) * w2;
    blur.xyz *= w_div;

    // Tone it down at the edges: only happens on first X pass!
