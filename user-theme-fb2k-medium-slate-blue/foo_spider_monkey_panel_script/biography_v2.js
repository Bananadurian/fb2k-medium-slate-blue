...
const JSON_DIR = window.GetProperty("biography.jsonDir", "D:\\11_MusicLib\\_Extras\\MusicMeta\\artists\\");
const ARTIST_COVER_DIR = window.GetProperty("biography.coverDir", "D:\\11_MusicLib\\_Extras\\MusicMeta\\covers\\artists\\original\\");
...
function parseArtistMbids(mbidStr) {
    if (!mbidStr) return [];
    return mbidStr.split(",").map(function(s) { return s.trim(); }).filter(function(s) { return s.length > 0; });
}
...
function reloadArtistData(metadb) {
        ...
        if (panelW > 0) {
            ...
            if (PANEL_CFG.showCover) {
                ...
                if (!carousel.images[index]) {
                    var changed = ensureCarouselImageReady(index, carousel, "same-artist-refresh");
                    manageCycleTimer();
                    if (changed) {
                        window.RepaintRect(0, 0, panelW, SEC.cover.rect.h);
                        coverReloaded = true;
    ...
    if (panelW > 0) {
        ...
        var pathsSig = cacheEntry.imgPaths && cacheEntry.imgPaths.length > 0 ? cacheEntry.imgPaths.join("||") : "fallback";
...
function updateCountryFlag() {
    if (!artistData || (!artistData.country && !artistData.countryCode)) { currentCountryFlagImg = null; lastCountryCode = null; return; }
