 ...
 */
const SUB_PANELS = {
    coverflowPanel: {key: "coverflowPanel", panelCaption: "Coverflow", panelIndex: 0, x: 0, y: 0, w: _scale(40), h: _scale(40), panel: null, missingLogged: false},
    spectrumAnalyzerPanel: {key: "spectrumAnalyzerPanel", panelCaption: "SpectrumAnalyzer", panelIndex: 1, x: 0, y: 0, w: _scale(40), h: _scale(40), panel: null, missingLogged: false},
    playlistViewPanel: {key: "playlistViewPanel", panelCaption: "PlaylistView", panelIndex: 2, x: 0, y: 0, w: _scale(40), h: _scale(40), panel: null, missingLogged: false},
};
...
// 子面板的padding
const PANEL_AREA_PADDING = normalizePadding({top:_scale(8), right:0, bottom:_scale(10), left:0});
...
function computeSubPanelRect(panelKey, layoutContext) {
    ...
    const containerTopHeight = layoutContext.containerTopHeight;
    const containerCenterHeight = layoutContext.containerCenterHeight;
    const centerStartY = layoutContext.centerStartY;
    const centerStartX = layoutContext.centerStartX;
    const containerBottomStartY = layoutContext.containerBottomStartY;

    // 上方的 coverflow
    if (panelKey === "coverflowPanel") {
        return {
            x: containerX,
            y: containerY,
            w: containerW,
            h: containerTopHeight,
        };
    }

    // 中间 Spectrum Analyzer
    if (panelKey === "spectrumAnalyzerPanel") {
        return {
            x: containerX,
            y: centerStartY,
            w: containerW,
            h: containerCenterHeight,
        };
    }

    // 底部 Playlist View
    if (panelKey === "playlistViewPanel") {
        return {
            x: containerX,
            y: centerStartY + containerCenterHeight,
            w: containerW,
            h: containerH - containerTopHeight - containerCenterHeight,
        };
    }
...
function layoutSubPanels() {
    ...
    // 几何参考线：三行 布局， 上 中 下比例 30% 20% 50%
    const containerTopHeight = Math.round(containerH * 0.35);
    const containerCenterHeight = Math.round(containerH * 0.15);
    const centerStartY = containerY + containerTopHeight;
    const containerBottomStartY = centerStartY + containerCenterHeight;
    ...
    const layoutContext = {
        ...
        containerTopHeight: containerTopHeight,
        containerCenterHeight: containerCenterHeight,
        centerStartY: centerStartY,
        containerBottomStartY: containerBottomStartY,
    };
